...
export const applyToJob = asyncHandler(async (req, res) => {
  ...
  if (!job || !job.isActive) {
    ...
    return res
      .status(404)
      .json({ success: false, error: { message: "Job not found or inactive" } });
  }
  ...
  if (existing) {
    ...
    return res
      .status(409)
      .json({ success: false, error: { message: "Already applied" } });
  }
  ...
  if (io)
    io.to(`user:${String(job.employer)}`).emit("application:created", {
      jobId: job._id,
      applicant: applicantId,
    });
...
export const updateApplicationStatus = asyncHandler(async (req, res) => {
  ...
  if (!app) {
    ...
    return res
      .status(404)
      .json({ success: false, error: { message: "Application not found" } });
  }
  ...
  const io = req.app.get("io");
  if (io)
    io.to(`user:${String(app.applicant)}`).emit("application:status_changed", {
...
export const withdrawApplication = asyncHandler(async (req, res) => {
  ...
  if (String(app.applicant) !== String(req.user.id))
    return res
      .status(403)
      .json({ success: false, error: { message: "Not allowed" } });
